...
public class StatisticServiceImp implements StatisticService {
    ...
    @Override
    public Map<String, Object> getRechargeAmount() {
        List<Map<String,Object>> rechargetAmount=statisticsDao.getRechargeAmount();
        int[]ints=DateUtils.getMonthByNearYear();
        /**
         * 这个是获取近一年的年月
         */
        ...
        for (int i = 0; i < mouth.length; i++) {
            if(ints[ints.length-1]<ints[i]){
                mouth[i]=nearYear-1+"."+ints[i];
            }else{
                mouth[i]=nearYear+"."+ints[i];
            }
        }

        String [] data1=new String[12];
        for(int i=0;i<ints.length;i++){
            for(int j=0;j<rechargetAmount.size();j++){
                if(ints[i] == Integer.parseInt(rechargetAmount.get(j).get("mon")+"")){
                    data1[i]=rechargetAmount.get(j).get("sum")+"";
                    break;
                }else{
                    data1[i]="0";
    ...
    @Override
    public Map<String, Object> getVipO() {
        List<Map<String, Object>> vipO = statisticsDao.getVipO();
        List list=new ArrayList();
        for(int i=0;i<vipO.size();i++){
            list.add(i,vipO.get(i).get("name"));
        }
        Map<String,Object> map=new HashMap<>();
        map.put("name",list);
        map.put("d",vipO);
